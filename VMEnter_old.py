...
VM_BYTECODE_SIZE = 4
ENTRY_JUMP_TARGET_REG = UC_X86_REG_RDI
VM_BYTECODE_PTR_REG = UC_X86_REG_RSI
VSP = UC_X86_REG_RBP
...
# TODO: this is jank, we should properly setup address space
...
print(f"vm bytecode location: {hex(mu.reg_read(VM_BYTECODE_PTR_REG))}")
print("vm bytecode content:")
print_bytes(idc.get_bytes(mu.reg_read(VM_BYTECODE_PTR_REG), VM_BYTECODE_SIZE))
print("next vm bytecode content:")
print_bytes(idc.get_bytes(mu.reg_read(VM_BYTECODE_PTR_REG) - 1, VM_BYTECODE_SIZE))
print(f"vsp (rbp) = rsp + {hex(mu.reg_read(VSP) - mu.reg_read(UC_X86_REG_RSP))}")
